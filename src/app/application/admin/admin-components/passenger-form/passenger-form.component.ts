...
export class PassengerFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.availableSeats$ = this.passengerForm.controls['seat_number'].valueChanges.pipe(
      startWith(''),
      map(value => this._filter(value))
    );
    if(this.data?.flight_number) this.passengerForm.get('flight_number')?.setValue(this.data?.flight_number);
    ...
    if(!this.data?.edit_access?.ancillary_services) this.passengerForm.controls['ancillary_services'].disable();
  }

  private _filter(value: string): string[] {
    const filterValue = value.toLowerCase();
    return this.availableSeats.filter(option => option.toLowerCase().includes(filterValue));
  }
